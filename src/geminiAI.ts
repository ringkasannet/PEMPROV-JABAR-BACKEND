...
import { createPrompt, asetPrompt, asetPromptDummy } from "./prompt";
...
export async function getUploadedFileToGemini(displayName: string) {
    const files = await listUploadedFileToGemini();
    console.log(
        `checking if ${displayName} exist in Gemini uploaded file list: ${files}`,
    );
    if (files) {
...
export async function uploadToGemini(pdfPath: string, fileName?: string) {
...
export async function checkActiveFiles(file: FileMetadataResponse) {
...
export async function queryAnalysis(query: string) {
...
export async function penjabaranPrompt(query: string, sources: BUMD[]) {
...
export async function penjelasanPrompt(query: string, sources: BUMD[]) {
...
export async function penjelasanPromptBulk(query: string, sources: BUMD[]) {
    console.log("fungsi penjelasanPrompt()");
...
export async function evaluasiAset(query: string, sources: any[]) {
...
export async function processGeminiWithFile(
    ...
    if (isJSON) {
        ...
        console.log(
            `processGeminiWithFile() ${content[0].fileData.fileUri}`
        );
