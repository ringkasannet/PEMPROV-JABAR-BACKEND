...
exports.router.get("/askDummy", async (req, res) => {
...
exports.router.get("/uploadChunksAset", async (req, res) => {
    console.log("halaman uploadChunksAset");
    const perdaName = chunksData[0].perda;
    try {
        const uploadChunks = await (0, asetHandler_js_1.uploadAsetChunksToMongo)(chunksData, perdaName);
        res.send(uploadChunks);
    }
    catch (error) { }
});
exports.router.get("/removeAsetPerda", async (req, res) => {
    ...
    try {
        await (0, asetHandler_js_1.removePerdaChunks)(perdaName);
        res.send(`${perdaName} chunks were removed from mongo db and pinecone vdb`);
    }
    catch (error) { }
});
exports.router.get("/getAllAset", async (req, res) => {
    console.log(`halaman getAllAset`);
    const listAset = await (0, asetHandler_js_1.getAllAset)();
    res.send(listAset);
});
exports.router.get("/processAsetEmbeddings", async (req, res) => {
    console.log(`halaman processAsetEmbeddings`);
    try {
        const embeddedChunks = await (0, asetHandler_js_1.processAsetEmbeddings)();
...
exports.router.post("/getAsetCandidate/:topK", async (req, res) => {
    console.log("halaman getAsetCandidate");
    const query = req.body.query;
    const topK = parseInt(req.params.topK);
    try {
        const asetCandidate = await (0, asetHandler_js_1.getAsetCandidate)(query, topK);
...
exports.router.post("/asetQA/:model/:topK", async (req, res) => {
    ...
    const query = req.body.query;
    try {
        const queryResult = await (0, asetHandler_js_1.processAsetQuery)(query, model, topK);
        if (!queryResult) {
        ...
        queryResult.map(async (itemStream) => {
            ...
            try {
                try {
                    for (var _d = true, _e = __asyncValues(itemStream.penjelasan.stream), _f; _f = await _e.next(), _a = _f.done, !_a;) {
                        _c = _f.value;
                        _d = false;
                        try {
                            const chunk = _c;
...
// input BUMD
exports.router.post("/admin/inputBUMD", async (req, res) => {
    console.log('halaman /admin/inputBUMD');
    const dataInput = req.body;
    // console.log(dataInput);
    await (0, bumdHandler_js_1.inputDataBUMDObject)(dataInput);
    res.send('done');
});
// hapus BUMD
exports.router.post("/admin/removeSelectedBUMD", async (req, res) => {
    console.log("halaman /admin/removeSelectedBUMD");
    const chunksID = req.body;
    // console.log(chunksID);
    await (0, bumdHandler_js_1.removeSelectedBUMD)(chunksID);
    res.send('done');
});
// input Aset
exports.router.post("/admin/inputAset", async (req, res) => {
    console.log("halaman /admin/inputAset");
    const dataInput = req.body;
    // console.log(dataInput);
    await (0, asetHandler_js_1.inputDataAsetObject)(dataInput);
    res.send('done');
});
// hapus Aset chunk
exports.router.post("/admin/removeSelectedAsetChunks", async (req, res) => {
    console.log("halaman /admin/removeSelectedAsetChunks");
    const chunksID = req.body;
    // console.log(chunksID);
    await (0, asetHandler_js_1.removeSelectedAsetChunks)(chunksID);
    res.send('done');
});
exports.router.get("/getAllBUMDID", async (req, res) => {
    console.log(`halaman getAllBUMDID`);
    const listBUMD = await (0, bumdHandler_js_1.getAllBUMD)();
    const listBUMDID = listBUMD.map(item => {
        return item._id;
    });
    res.send(listBUMDID);
});
exports.router.get("/getAllBUMDVector", async (req, res) => {
    console.log('halaman getAllBUMDVector');
    try {
        const result = await (0, pinecone_js_1.getAllVector)("bumd");
        res.send(result);
    }
    catch (error) {
...
exports.router.get("/getAllAsetID", async (req, res) => {
...
exports.router.get("/getAllAsetVector", async (req, res) => {
    console.log('halaman getAllAsetVector');
    try {
        const result = await (0, pinecone_js_1.getAllVector)("aset");
        res.send(result);
    }
    catch (error) {
        res.send(error.message);
