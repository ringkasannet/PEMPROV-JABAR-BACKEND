...
exports.asetRouter = void 0;
const express_1 = __importDefault(require("express"));
const asetHandler_1 = require("../asetHandler");
const asetHandler_2 = require("../asetHandler");
const asetHandler_3 = require("../asetHandler");
const asetHandler_4 = require("../asetHandler");
exports.asetRouter = express_1.default.Router();
const chunksData = [
    {
...
exports.asetRouter.get("/uploadChunksAset", async (req, res) => {
    console.log("halaman uploadChunksAset");
    const perdaName = chunksData[0].perda;
    try {
        const uploadChunks = await (0, asetHandler_4.uploadAsetChunksToMongo)(chunksData, perdaName);
        res.send(uploadChunks);
    }
    catch (error) { }
});
exports.asetRouter.get("/removeAsetPerda", async (req, res) => {
    ...
    try {
        await (0, asetHandler_3.removePerdaChunks)(perdaName);
        res.send(`${perdaName} chunks were removed from mongo db and pinecone vdb`);
    }
    catch (error) { }
});
exports.asetRouter.get("/all-chunks", async (req, res) => {
    console.log(`halaman getAllAset`);
    const asetChunks = await (0, asetHandler_2.getAllAsetChunks)();
    res.send(asetChunks);
});
exports.asetRouter.get("/all-perda", async (req, res) => {
    console.log(`halaman getAllAset`);
    const asetChunks = await (0, asetHandler_2.getAllAsetChunks)();
    const listPerda = (0, asetHandler_1.mergeChunks)(asetChunks);
    res.send(listPerda);
});
exports.asetRouter.get("/processAsetEmbeddings", async (req, res) => {
    console.log(`halaman processAsetEmbeddings`);
    try {
        const embeddedChunks = await (0, asetHandler_1.processAsetEmbeddings)();
...
exports.asetRouter.post("/aset-candidates/:topK", async (req, res) => {
    console.log("halaman getAsetCandidate");
    const query = req.body.query;
    const topK = parseInt(req.params.topK);
    try {
        const asetCandidate = await (0, asetHandler_1.getAsetCandidate)(query, topK);
...
exports.asetRouter.post("/q-a/:model/:topK", async (req, res) => {
    ...
    const query = req.body.query;
    try {
        const queryResult = await (0, asetHandler_1.processAsetQuery)(query, model, topK);
        if (!queryResult) {
...
//# sourceMappingURL=asetRouter.js.map